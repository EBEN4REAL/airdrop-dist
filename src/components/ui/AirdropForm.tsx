...
import { useRadixToast } from "@/components/ui/ToastRoot";
...
export default function AirdropForm({
  ...
  const writeContract = async (tSenderAddress?: `0x${string}`) => {
    await writeContractAsync({
      ...
      toast("Success", "Airdrop transaction submitted successfully.");
  }
  ...
  async function handleSubmit() {
    ...
    if (result < total) {
      ...
      if (tSenderAddress) {
        await writeContract(tSenderAddress as `0x${string}`);
      }
    } else {
      await writeContract();
    }
  ...
  useEffect(() => {
    if (tokenData?.[0]?.result !== undefined) {
      const decimals = tokenData[0].result as number;
      const formattedTotal = formatTokenAmount(total, decimals);
      setFormattedTotal(formattedTotal);
    } else {
      setFormattedTotal("0.00");
    }
  }, [total, amounts, tokenData]);
